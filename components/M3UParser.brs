...
sub runTask()
    ...
    m3uUrls = [
        ...
        "https://sasan.jafarzadeh-sasan-tru.workers.dev/playlist.m3u"
    ]
            ...
            while true
                ...
                if type(msg) = "roUrlEvent"
                    if msg.GetResponseCode() = 200
                        newContent = msg.GetString()
                        if newContent <> invalid and newContent <> ""

                            if response <> ""
                                response = response + chr(10) + newContent
                            else
                                response = newContent
                            end if
                        end if
                    else
                        print "HTTP ERROR "; msg.GetResponseCode(); " for "; m_url
                    end if
                    exit while
                end if

                if msg = invalid
                    print "timeout for "; m_url
                    exit while
                end if
            end while
        end if
    ...
    for each g in groupOrder
        found = false
        ogCount = orderedGroups.Count()
        for i = 0 to ogCount - 1
