' --- اصلاح شده: تغییر نام تابع برای جلوگیری از تداخل با تابع پایین ---
function intToStringTrimmed(n as Integer) as String
    s = stri(n)
    result = ""
    for k = 1 to Len(s)
        c = Mid(s, k, 1)
...
sub doSave()
    ...
    ' --- اصلاح شده: استفاده از زمان برای ساخت کلید یکتا و جلوگیری از بازنویسی ---
